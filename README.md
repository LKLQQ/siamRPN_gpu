...
| 资源                   |  RTX3090 |
| 数据集                 | VID-youtube-bb|
| 训练参数   | epoch=50, steps=1417, batch_size = 32  |
| 优化器                  |SGD  |
| 损失函数 | 自定义损失函数 |
| 输出              | 目标框  |
| 损失             | 100~0.05     |
| 速度 |  8卡：296毫秒/步  |
| 总时长 | 8卡： 5.8小时|
| 调优检查点 |   247.44MB（.ckpt 文件）|
| 脚本                | [siamRPN脚本](https://gitee.com/mindspore/models/tree/master/research/cv/siamRPN) |

### 评估性能

| 参数  |  siamRPN(GPU)         | siamRPN(GPU)                   |
| ------------------- |--------------------------- | --------------------------- |
| 模型版本      |simaRPN                       | simaRPN                       |
| 资源        ||GPU         | GPU                       |
| 上传日期               |     2021-12-7      |         2021-12-7             |
| MindSpore版本   |       1.5.0   |   1.5.0   |
| 数据集 | |vot2015，60个video          | vot2016，60个video            |
| batch_size          |   |1           | 1                |
| 输出 | |目标框             | 目标框        |
| 准确率 |单卡：accuracy：0.5826,robustness：0.298,eao:0.329;       | 单卡：accuracy：0.5538,robustness：0.345,eao:0.295;                  |
